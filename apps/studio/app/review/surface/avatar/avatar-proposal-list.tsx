...
export function AvatarProposalList() {
  const [proposals, setProposals] = useState<Proposal[]>([]);
  const [loading, setLoading] = useState(true);
  const [approving, setApproving] = useState<string | null>(null);

  useEffect(() => {
    fetch("/api/proposals?lane_type=surface&target_type=avatar_candidate")
      .then((r) => r.json())
  ...
  if (proposals.length === 0) return <p>No avatar proposals pending.</p>;
  return (
