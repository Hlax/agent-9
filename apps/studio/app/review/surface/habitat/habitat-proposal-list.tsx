...
export function HabitatProposalList({ view }: { view: "pending_review" | "approved" | "archived" }) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/proposals?lane_type=surface&target_type=public_habitat_proposal&proposal_state=${view}`)
      .then((r) => r.json())
  ...
  if (proposals.length === 0) return <p>No habitat proposals in this view.</p>;
  return (
