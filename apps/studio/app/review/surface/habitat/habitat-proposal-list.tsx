...
export function HabitatProposalList() {
  const [proposals, setProposals] = useState<Proposal[]>([]);
  const [loading, setLoading] = useState(true);
  const [approving, setApproving] = useState<string | null>(null);

  useEffect(() => {
    fetch("/api/proposals?lane_type=surface&target_type=public_habitat_proposal")
      .then((r) => r.json())
  ...
  const handleApprove = async (id: string) => {
    setApproving(id);
    try {
      const res = await fetch(`/api/proposals/${id}/approve`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "approve_publication" }) });
  ...
  if (proposals.length === 0) return <p>No habitat proposals pending.</p>;
  return (
        ...
        <li key={p.proposal_record_id} style={{ border: "1px solid #ccc", borderRadius: 8, padding: "1rem", marginBottom: "0.5rem" }}>
          ...
          {p.proposal_state === "pending_review" && (
            <button type="button" onClick={() => handleApprove(p.proposal_record_id)} disabled={approving === p.proposal_record_id} style={{ marginTop: "0.5rem" }}>
              {approving === p.proposal_record_id ? "Approving…" : "Approve for publication"}
            </button>
          )}
        </li>
      ))}
