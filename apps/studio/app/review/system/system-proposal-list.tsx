...
export function SystemProposalList({ view }: { view: "pending_review" | "approved" | "archived" }) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/proposals?lane_type=system&proposal_state=${view}`)
      .then((r) => r.json())
  ...
  if (proposals.length === 0) return <p>No system proposals in this view.</p>;
  return (
...
export function SystemProposalTabs({ view }: { view: "pending_review" | "approved" | "archived" }) {
  return (
    <nav style={{ display: "flex", gap: "1rem", marginBottom: "1rem" }}>
      {VIEWS.map((v) => (
        <a
          key={v.view}
          href={`/review/system${v.view === "pending_review" ? "" : `?view=${v.view}`}`}
